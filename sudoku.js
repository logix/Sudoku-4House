...
function checkBlock(row, col, x) {
  var startRow = Math.floor(row / sx) * sx;
  var startCol = Math.floor(col / sy) * sy;
  for (var i = 0; i < sx; i++) {
    for (var j = 0; j < sy; j++) {
...
function blockOK(n) {
  var startRow = Math.floor((n - 1) / sx) * sx;
  var startCol = (n - 1) % sy * sy;
  var sum = 0;
  for (var i = 0; i < sx; i++) {
    for (var j = 0; j < sy; j++) {
...
function setBlockRandomly(n) {
  ...
  for (var i = 0; i < sx; i++) {
    for (var j = 0; j < sy; j++) {
      sudoku[startRow + i][startCol + j] = a[i * sx + j]; // 只有一个sx？
    }
  }
}
  ...
  for (var i = 0; i < size; i++) {
    for (var j = 0; j < size; j++) {
      if (a[i][j] !== 0) {
        ...
        table[i][j].innerHTML = '<input onblur="unHint(' + i + ',' + j + ');" onclick="loadHint(' + i + ',' + j + ');" type="text" maxlength="1" onchange="onInput(' + i + ',' + j + ');"/>';
      }
    }
  }
}

function loadHint(i, j) {
  var clickElement = table[i][j].firstElementChild;
  clickElement.style.backgroundColor="Wheat";
  // for(var idx = 0; idx < size; idx++){
  //   console.log(i,idx)
  //   var clickElement = table[i][idx].firstElementChild;
  //   clickElement.style.backgroundColor="Wheat";
  // }
...
function unHint(i, j) {
  var clickElement = table[i][j].firstElementChild;
  clickElement.style.backgroundColor="#fff";
}
...
function createGame() {
  ...
  for (var i = 0; i < size; i++) {
    for (var j = 0; j < difficulty + Math.floor(Math.random() * 2); j++) {
      sudoku[i][Math.floor(Math.random() * size)] = 0;
    }
  }
}
// 设置难度为简答
